...
class VolatilityAdaptiveStrategy(BaseStrategy):
    ...
    def __init__(self, simulator, params=None):
        ...
        # Default parameters
        self.short_window = params.get('short_window', 8)  # Window for short-term volatility
        self.medium_window = params.get('medium_window', 20)  # Window for medium-term volatility
        self.long_window = params.get('long_window', 40)  # Window for long-term volatility
        self.min_samples = params.get('min_samples', self.medium_window)  # Minimum samples before making predictions
        self.high_volatility_threshold = params.get('high_volatility_threshold', 0.6)  # Threshold for high volatility
        self.low_volatility_threshold = params.get('low_volatility_threshold', 0.3)  # Threshold for low volatility
        self.confidence_threshold_base = params.get('confidence_threshold_base', 0.55)  # Base confidence threshold
        self.confidence_scaling = params.get('confidence_scaling', 0.2)  # How much to adjust confidence by volatility
        self.banker_bias = params.get('banker_bias', 0.01)  # Slight bias towards banker bets
        self.use_adaptive_window = params.get('use_adaptive_window', True)  # Whether to adjust analysis window size
        self.statistical_mode = params.get('statistical_mode', 'frequency')  # Statistical method for prediction
        self.pattern_length = params.get('pattern_length', 3)  # Length of pattern to consider in pattern mode
        self.min_pattern_occurrences = params.get('min_pattern_occurrences', 2)  # Min occurrences for valid pattern

...
import time
from core.analysis import history_template_match, match_on_color
import numpy as np
...
def get_bet_allowed_coordinates(threshold=0.8):
    ...
    if bet_allowed_coordinates[0] is None:
        ...
        if match_on_color(np.array(img), {"Allowed": (106, 58, 141), "Not Allowed": (29, 26, 23)}) == "Allowed":
...
def capture_history(running, threshold=0.8):
    ...
    outcomes = history_template_match(cropped_image)
    return outcomes